...
%In this chapter the used random generation methods are presented and the created blending process is explained in detail. \par
...
%		{
%			& |[source] (base-surface)| \shortstack{Load \\ Base Surface} & \\ 
%			|[process] (random)| \shortstack{ Generate  \\ Random Surface } &  &[row sep=1cm] |[process] (norm)| Normalize \\
%			|[process] (unsharp)| \shortstack{Gaussian Blur \\ \& Subtraction} & &[row sep=1cm] |[process] (map)| \shortstack{Map According to \\ Hermite Spline} \\
%			& |[process] (multiply)| Multiply & \\
%			& |[process] (add)| Add Base Surface & \\
%			& |[process] (norm2)| Normalize & \\
%			& |[source] (result) | \shortstack{Result Surface \\ \& Normal Map} & \\
%		};
%		
%		\draw[to] (base-surface) -| node[pos=0.25, above] {Size} (random);
%		\draw[to] (random) -- (unsharp);
%		\draw[to] (unsharp) |- node[pos=0.5, below] {\shortstack{ Random Surface  Details }} (multiply);
%		
%		\draw[to] (base-surface) -| node[pos=0.25, above] {Surface} (norm);
%		\draw[to] (norm) -- (map);
%		\draw[to] (map) |- node[pos=0.5, below] {\shortstack{ Adjusted Base Surface }} (multiply);
%		
%		
%		\draw[to] (multiply) -- (add);
%		\draw[to] (add) -- (norm2);
%		\draw[to] (norm2) -- (result);
%		\end{tikzpicture}
%	\end{center}
...
\begin{landscape}
	...
	\begin{table}[h!] \centering \footnotesize
		...
		\begin{tabularx}{\linewidth}{|c|c|c|b|c|c|}
			\hline
			\multicolumn{2}{|c|}{\textbf{Phase}} & \heading{Name} & \heading{Description} & \heading{Minimum} & \heading{Maximum} \\ 
			\hline \hline
			\multirow{5}{*}{\bfseries\shortstack{Random Surface  \\ Generation}} & \textbf{Fourier Filtering} & Filter Power & $\beta$ value & 1.0 & 2.9 \\
			\cline{2-6} & \multirow{4}{*}{\textbf{Noise Synthesis}} & Octaves & Number of noise samples & & \\
			\cline{3-6} & & Lacunarity & Gap between successive frequencies & & \\
			\cline{3-6} & & Persistence & Contribution gap between successive octaves & & \\
			\cline{3-6} & & Base & Base frequency value & & \\
			\hline
			\multicolumn{2}{|c|}{
				\multirow{2}{*}{\bfseries\shortstack{Blending}}
			}
			& Spline Control Points & & &  \\
			\cline{3-6} \multicolumn{2}{|c|}{} & Blend Strength & & & \\
			\hline
